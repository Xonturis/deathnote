...
const get_execution = require('./getexecution')
const config = require('../../config.json')

async function get_cookies_and_redirect() {
    return new Promise(async (resolve, reject) => {
        const execution = await get_execution()
        const username = config.username
        const password = config.password

        const data = new URLSearchParams(`username=${username}&password=${password}&otpPassword=&u2fSignature=&execution=${execution}&_eventId=submit&geolocation=`)
...
module.exports = get_cookies_and_redirect